...
# 1) run make once to create and update the po/visualitemmodel1.pot
# 2) copy the template file and set the name to the language you want to
#    translate to:   cp po/visualitemmodel1.pot po/en.po
# 3) edit the po file
...
APP_ID=visualitemmodel1.liu-xiao-guo
...
po/visualitemmodel1.pot: $(QMLJS_FILES)
	mkdir -p $(CURDIR)/po && xgettext -o po/visualitemmodel1.pot --qt --c++ --add-comments=TRANSLATORS --keyword=tr --keyword=tr:1,2 $(QMLJS_FILES) --from-code=UTF-8
